...
class BloqueoInstancia:
    ...
    def esperar(
        ...
        *,
        ...
        n = 0
        while not self.intentar():
            n += 1
            if intentos is not None and n >= intentos:
                raise TimeoutError("Otra instancia del asistente sigue activa")
            log.warning("Otra instancia del asistente está activa: en espera (reintento en %s s)", espera_s)
            dormir(espera_s)
    ...
    def liberar(self) -> None:
        if self._conn is not None:
            try:
                self._conn.close()  # al cerrar la sesión, Postgres suelta el bloqueo
            except psycopg.Error:
                pass
            self._conn = None
